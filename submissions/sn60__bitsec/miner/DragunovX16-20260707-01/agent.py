...
def agent_main(
    ...
    static_findings = _static_audit(files)
    model_findings = _model_audit(files, inference_api)
    findings = _dedupe_findings(model_findings + static_findings)
...
def _model_audit(
    ...
    overview_prompt = _build_overview_prompt(files)
    if overview_prompt:
        try:
            overview_text = _call_model(endpoint, overview_prompt)
            calls_used += 1
            findings.extend(_findings_from_model_text(overview_text))
        except BudgetExhausted:
            return findings
        except RuntimeError:
            pass

    for target in _deep_targets(files, findings):
        if calls_used >= MAX_MODEL_CALLS or time.monotonic() > deadline:
            break
        prompt = _build_deep_prompt(target, files)
        try:
            ...
            continue
        findings.extend(_findings_from_model_text(text, default_file=str(target["path"])))
